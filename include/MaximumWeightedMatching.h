...
class MaximumWeightedMatching {
  ...
  public:
    MaximumWeightedMatching(int N, const std::vector<InputEdge> &in) :
        N(N), B((N - 1) / 2), S(N + B + 1), ofs(N + 2), edges(in.size() * 2), heap2(S), heap2s(S, S), heap3(edges.size()), heap4(S) {
    ...
    tcost_t maximum_weighted_matching(std::vector<std::pair<int, int>> &matching, bool init_matching = false) {
      initialize();
      set_potential();
      if (init_matching)
  ...
  private:
        ...
        if (mate[u] > u) {
          ...
          for (int eid = ofs[u]; eid < ofs[u + 1]; ++eid) {
            if (edges[eid].to == mate[u])
              max_c = std::max(max_c, edges[eid].cost);
          }
          ret += max_c;
        }
      return ret >> 1;
    ...
    inline tcost_t reduced_cost(int u, int v, const Edge &e) const { return tcost_t(potential[u]) + potential[v] - e.cost; }
    ...
    bool adjust_dual_variables(int root) {
      ...
      // -- events --
      cost_t time_next = std::min({time1, time2, time3, time4});
